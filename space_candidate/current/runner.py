...
def release_candidate_gate() -> dict[str, object]:
    ...
    required_entrypoints = [candidate / "README.md", candidate / "logbook.json", candidate / "pages/index.md"]
    if not all(path.is_file() for path in required_entrypoints):
        raise AssertionError("candidate canonical entrypoint is incomplete")

    logbook = json.loads((candidate / "logbook.json").read_text(encoding="utf-8"))
    if logbook["space_id"] != "DineshAI/Iww9TICvKj":
        raise AssertionError("candidate targets the wrong Space")
    if logbook["root"]["children"][0]["slug"] != "current-verification":
        raise AssertionError("current verifier is not first in navigation")

    navigation_files: list[Path] = []

    def collect_navigation(node: dict[str, object]) -> None:
        navigation_files.append(candidate / str(node["file"]))
        for child in node.get("children", []):
            collect_navigation(child)

    collect_navigation(logbook["root"])
    opened = set(required_entrypoints + navigation_files)
    markdown_link = re.compile(r"\[[^\]]*\]\(([^)]+)\)")
    queue = list(opened)
    while queue:
        path = queue.pop()
        if not path.is_file():
            raise AssertionError(f"canonical traversal found missing file: {path.relative_to(candidate)}")
        ...
        for target in markdown_link.findall(path.read_text(encoding="utf-8")):
            ...
            try:
                linked.relative_to(candidate.resolve())
            except ValueError as exc:
                raise AssertionError(f"link escapes candidate: {target}") from exc
            if linked not in opened:
                opened.add(linked)
                queue.append(linked)

    claim_requirements = {
        1: ["Exact claim", "Assumptions", "Raw observed evidence", "runner.py", "verify.py", "Negative", "BLOCKED", "Confidence: LOW"],
        2: ["Exact claim", "Assumptions", "Raw observed evidence", "runner.py", "verify.py", "equal-timescale", "BLOCKED", "Confidence: LOW"],
        3: ["Exact claim", "Assumptions", "Raw observed evidence", "runner.py", "verify.py", "terminal bound", "BLOCKED", "Confidence: LOW"],
        4: ["Exact claim", "Exact algorithm", "Raw observed evidence", "runner.py", "verify.py", "rejected", "BLOCKED", "Confidence: LOW"],
        5: ["Exact claim", "Exhaustive method", "raw evidence", "runner.py", "verify.py", "destructive controls", "VERIFIED", "Confidence: HIGH"],
    }
    for claim, tokens in claim_requirements.items():
        page = candidate / f"pages/current-claim-{claim}/page.md"
        text_value = page.read_text(encoding="utf-8")
        missing = [token for token in tokens if token.lower() not in text_value.lower()]
        if missing:
            raise AssertionError(f"Claim {claim} page is missing evaluator-visible fields: {missing}")

    overview = (candidate / "pages/current-verification/page.md").read_text(encoding="utf-8")
    for claim in range(1, 6):
        if f"| {claim} | `current-claim-{claim}` | Complete | Complete | Complete | Complete | Complete | Complete |" not in overview:
            raise AssertionError(f"Claim {claim} visibility row is incomplete")
    if "Historical rejected baseline" not in overview:
        raise AssertionError("historical verifier is not explicitly rejected")

    for raw_path in sorted((candidate / "raw").glob("*.json")):
        json.loads(raw_path.read_text(encoding="utf-8"))
    for svg in sorted((candidate / "reports/current/images").glob("*.svg")):
        ET.fromstring(svg.read_text(encoding="utf-8"))

    protected = ROOT / ".openresearch/protected/JUDGED_SPACE_MANIFEST.sha256"
    archival_paths = {
    ...
    for line in protected.read_text(encoding="utf-8").splitlines():
        expected, old_path = line.split(maxsplit=1)
        relative = old_path.removeprefix("./")
        current_path = candidate / relative
        if not current_path.exists():
            raise AssertionError(f"judged file was deleted: {relative}")
        preserved_path = candidate / archival_paths.get(relative, relative)
        actual = hashlib.sha256(preserved_path.read_bytes()).hexdigest()
        if actual != expected:
            raise AssertionError(f"judged evidence changed without byte-preserved archive: {relative}")
        protected_count += 1
    ...
    if len(allowlist) != len(set(allowlist)) or allowlist != sorted(allowlist):
        raise AssertionError("upload allowlist must be unique and sorted")
    manifest = {}
    for line in (candidate / "SHA256SUMS.txt").read_text(encoding="utf-8").splitlines():
        digest, relative = line.split(maxsplit=1)
        manifest[relative] = digest
    if "SHA256SUMS.txt" not in allowlist or set(manifest) != set(allowlist) - {"SHA256SUMS.txt"}:
        raise AssertionError("hash manifest must cover every allowlisted file except itself")
    for relative in allowlist:
        upload = candidate / relative
        if not upload.is_file():
            raise AssertionError(f"allowlisted upload missing: {relative}")
        upload.read_text(encoding="utf-8")
        if relative != "SHA256SUMS.txt" and hashlib.sha256(upload.read_bytes()).hexdigest() != manifest[relative]:
            raise AssertionError(f"upload hash mismatch: {relative}")
    ...
    for relative in allowlist:
        text_value = (candidate / relative).read_text(encoding="utf-8")
        if any(pattern.search(text_value) for pattern in secret_patterns):
            raise AssertionError(f"possible secret in allowlisted file: {relative}")

    missing_opened = sorted(str(path.relative_to(candidate)) for path in opened if not path.is_file())
    if missing_opened:
        raise AssertionError(f"evaluator-blind traversal found missing files: {missing_opened}")
    current_verifier_visible = {
        str((candidate / "current/runner.py").resolve()),
        str((candidate / "current/verify.py").resolve()),
    }.issubset({str(path) for path in opened})
    if not current_verifier_visible:
        raise AssertionError("current executable verifier is not reachable from the canonical entrypoint")

    return {
        "status": "PASS",
        "target_space": "DineshAI/Iww9TICvKj",
        "protected_judged_files_preserved": protected_count,
        "upload_allowlist_count": len(allowlist),
        "canonical_files_opened": sorted(str(path.relative_to(candidate)) for path in opened),
        "missing_conclusions": [],
