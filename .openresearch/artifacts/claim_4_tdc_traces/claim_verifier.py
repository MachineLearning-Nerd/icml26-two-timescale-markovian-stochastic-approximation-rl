...
def verify(results: dict) -> None:
    ...
    release = results["release_candidate_gate"]
    if release["status"] != "PASS":
        raise AssertionError("release candidate gate failed")
    if release["target_space"] != "DineshAI/Iww9TICvKj":
        raise AssertionError("release candidate targets a second or incorrect Space")
    required_release_flags = {
        "visibility_matrix_complete",
        "current_verifier_obvious",
        "old_file_set_subset",
    }
    if not all(release[name] for name in required_release_flags):
